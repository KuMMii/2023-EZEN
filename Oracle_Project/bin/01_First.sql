...
--Table:데이터 베이스에서 사용되는 데이터 집합의단위
--열:*Field*,속성, 애트리뷰트
--행:*Record*,튜플

--위와같이 표형식의 데이터베이스를 관계형 데이터 베이스라고 부름

--현재 출시되어 데이터베이스로 활영되는 제품들 : 오라클, MySQL,MSSQL,MariaDB,MongoDB,Access등
--데이터베이스의 조작 운영을 가능하게 하는 프로그램:DBMS(DataBase Management System)
--각 데이터베이스 제품들에는 자신의 데이터베이스를 관리할 수 있는 DBMS가 존재함
--오라클데이터 베이스에서 사용가능한 DBMS: SQL Developer, SQL PLUS, 이클립스 등

--데이터 베이스의 활용 분야 : 일반웹사이트 게시판의 게시물 저장용, 포털검색 사이트 검색 대상정보 저장용, RPG 등 
--게임의 게임정보 또는 캐릭터 정보,회원들의 회원정도 등


--데이터베이스에 활용되는 언어(Language)
--SQL(Structured Query Language): 관계형 데이터베이스 관리 시스템(RDBMS)의 데이터를 관리하기 위해 설계된 특수목적의 프로그래밍 언어
...
--Select:테이블에 있는 레코드 중 일부 또는 전부를 조회(검색)해 열람하기 위한 명령
...
--기본키(Primary Key):테이블을 구성하는 필드들 중에서 갖는 값들이 빈칸(null)이 없고 서로 다른 값(유일한 값)들을 갖고 있어
--레코드들을 유일하게 구분해 낼 수 있는 필드
