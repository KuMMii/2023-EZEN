...
--테이블의 이름은 객체를 의미할 수 있는 적절한 이름으로 사용함(자바 변수 이름 규칙과 거의 동일)
...
--	필드명 옆에 현재 필드에만 적용하는 제약조건을 필드수준의 제약이라고 함
-- 아래는 테이블 수준의 제약조건
...
--제약조건(CONSTRAINT)
--PRIMARY KEY
-- -테이블에 저장된 레코드를 고유하게 식별하기 위한 키, 하나의 테이블에 하나의 기본키만 정의할 수 있음
-- -여러 필드가 조합된 기본키 생성 가능
-- -기본키는 중복된 값을 가질 수 없으며 빈칸도 있을 수 없음
...
-- CHECK
-- -입력할 수 있는 값의 범위를 제한한다. CHECK제약으로는 TRUE or FALSE로 평가할 수 있는 논리식을 지정함
--FORIEGN KEY
-- -관계형 데이터 베이스에서 테이블 간의 관례를 정의하기 위한 기본키를 다른 테이블의 외래키로
...
--drop table 테이블명 purge; -->테이블 지우는 명령
...
rent_date date default sysdate , --sysdate:오늘날짜를 넣어주는 키워드  //대여날짜
...
constraint rentlist_pk primary key(rent_date,indexk),
constraint fk1 foreign key(bnum) references booklist(booknum),
--현재 테이블의 bnum필드는 booklist 테이블의 booknum 필드값을 참조함
constraint fk2 foreign key(mnum) references memberlist(membernum)
--현재 테이블의 mnum필드는 memberlist 테이블의 membernum 필드값을 참조함
);
select*from rentlist;

--rent-date와 indexk를 조합해 기본키(rent_pk)를 생성. 두개의 필드가 조합되어 기본키로 지정될 수 있음. 이를 슈퍼키라고 함
--rentlist테이블의 bnum은 booklist 리스트에 booknum을 참조하는 외래키
