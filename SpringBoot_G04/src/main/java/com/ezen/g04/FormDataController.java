...
public class FormDataController {
	...
	public String test2(@RequestParam("id") String id,
		...
		//RequestParam 사용 특성
		//1. 정수형 자료를 정수형 변수에 Integer.parseInt없이 바로 저장 가능
		//2. 이때, 전달되는 자료의 형태는 반드시 동일해야 에러가 없음
		//3. 전달값이 없으면(null), 에러가 발생
...
//		String name=request.getParameter("name");
	...
	public String test3(@ModelAttribute("mem") MemberDto memberdto){
	      // 파라미터와 일치하는 멤버변수가 있는 객체를 만들고 이 객체를 매개변수로 사용합니다
	      // 전달된 파라미터는 매개변수에 자동 입력됩니다
	      // ModelAttribute 어노테이션을 통해 자동으로 Model에도 저장됩니다
	      // 위 매개변수에 사용한 어노테이션은 Model.addAttribute("mem", member) 라고 쓴것과 같습니다
		return "test3";
	}
