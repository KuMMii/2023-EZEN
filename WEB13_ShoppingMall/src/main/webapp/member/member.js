...
function go_next(){
	...
	//자바스크립트에서 jsp페이지 내의 radio버튼을 바라볼때, 같은 name의 okon인 것이 여러개면 name값에 의한 배열로 인식돼 사용됨
	...
	if(document.contractFrm.okon[1].checked==true){
		...
		//contractFrm 폼에 action도 없고 submit버튼도 없는 것을 스크립트 명령으로 action값을 설정하고 submit()메서드로 이동까지 실행
	}
}
...
function post_zip(){
	...
	var opt="toolbar=no, menubar=no, resizable=no, scrollbars=no, width=600, height=400, top=300, left=300";
