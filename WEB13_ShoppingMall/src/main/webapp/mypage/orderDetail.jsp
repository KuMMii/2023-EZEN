...
<h2> My Page(주문 상세 정보)</h2>
	<form>
		...
		<table id="cartList">
			...
			<tr>
				...
				<td><fmt:setLocale value="ko_KR"/><fmt:formatNumber value="${totalPrice}" type="currency"/></td>
			</tr>
			<tr>
				<td colspan="4">배송주소 : ${orderDetail.zip_num} ${orderDetail.address1 }${orderDetail.address2}</td>
			</tr>
	...
	<h3>주문 상품 정보</h3>
	<table id="cartList">
		<tr><th>상품평</th><th>상품별주문번호</th><th>수량</th><th>가격</th><th>처리 상태</th></tr>
		<c:forEach items="${orderList}" var="orderVO">
			<tr><td>${orderVO.pname}</td><td>${orderVO.odseq}</td>
			<td>${orderVO.quantity}</td>
			<td><fmt:setLocale value="ko_KR"/><fmt:formatNumber type="currency" value="${orderVO.price2*orderVO.quantity}"/></td>
			<td>
				<c:choose>
