...
public class MemberDao {
	...
	public int insertMember(MemberVO mvo) {
		int result=0;
		con=Dbman.getConnection();
		String sql="insert into member(id, pwd, name, phone, zip_num, address1, address2, email) values(?,?,?,?,?,?,?,?)";
		try {
			pstmt=con.prepareStatement(sql);
	...
	public void updateMember(MemberVO mvo) {
		con=Dbman.getConnection();
		String sql="update member set pwd=?, name=?, phone=?, zip_num=?, address1=?, address2=?, email=? where id=?";
