...
--greatest(x,y,z....)-주어진 숫자 중에서 가장 큰 수
--least(x,y,z...)-주어진 숫자 중 가장 작은 수 
...
--MID('문자열',시작위치, 개수)-문자열 중 시작위치부터 개수만큼을 출력
...
--DAYOFWEEK(날짜)-날짜의 주별 일자 출력(일요일(1),월요일(2)...)
--WEEKDAY(날짜)-날짜의 주별일자 출력(월요일(0),화요일(1)...)
...
-- 오라클의 to_DATE('20200405', 'YYYYMMDD') 문자->날짜

--MYSQL
--date_format(now(), '%Y%m%d')
--str_to_date('20230420', '%Y%m%d')

select date_format(now(), '%Y%m%d')from dual;
select str_to_date('20230420', '%Y%m%d')from dual;
