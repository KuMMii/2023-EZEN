...
//		String sql = "select * from board order by num desc";
		String sql="select * from( select * from( select rownum as rn, b.* from( ( select * from board order by num desc) b) )"
				+ " where rn>=? ) where rn<=?";
	...
	public void insertBoard(BoardDto bdto) {
		con = Dbman.getConnection();
		String sql = "insert into board(num, userid, pass, title,email, content,imgfilename) values(board_seq.nextVal,?,?,?,?,?,?)";
