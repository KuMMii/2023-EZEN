...
public class MultiFileUpload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MultipartRequest multi = new MultipartRequest(
				...
				);//폼안에 있던 <input type="file">의 파일들은 MultipartRequest 생성시 한번에 업로드 됨
		
		//한번에 전달된 "파일이름들"은 Enumeration을 사용해 전달받고, 하나씩 꺼내서 사용
		Enumeration files = multi.getFileNames(); //전송된 input type="file"의 name들
		//파일(<input type="file" name="uploadFile01">)들을 Enumeration에 저장
		int i =1;
		while(files.hasMoreElements()) {//파일 요소의 개수만큼 반복실행
			...
			request.setAttribute("ori_system_name"+i, ori_file_name);
			i++;
		}
