...
public class OrderService {
	...
	public HashMap<String, Object> insertOrder(String id) {
		...
		//3. 그 주문번호와 함께 카트리스트에 있는 상품들을 하나하나 order_detail 테이블에 추가
		//		-CartList를 아이디로 한번 더 조회함
		//4. 카트테이블에서 방금 주문한 상품들을 삭제함
		...
		//5. 주문리스트와 주문번호를 해시맵에 담기
		result.put("oseq", oseq);
		result.put("totalPrice", list);
	...
	public int orderInsertOne(int pseq, String id, int quantity) {
		int oseq=0;
		odao.insertOrders(id);
		oseq=odao.LookupMaxOseq();
		odao.OrderDetailInsertOne(oseq,pseq,quantity);
		return oseq;
	}
	...
	public ArrayList<OrderVO> getFinalListIng(String id) {
		...
		for(int oseq: oseqList) {
			...
			//4. orderListIng에서 첫번째 주문을 따로 저장함
			OrderVO ovo=orderListIng.get(0 );
			
			//5. orderListIng의 주문들의 요약정보를 ovo에 저장함
			if(orderListIng.size()==1) {
				ovo.setPname(ovo.getPname());
				ovo.setPrice2(ovo.getPrice2()*ovo.getQuantity());
	...
	public ArrayList<OrderVO> getFinalListAll(String id) {
		...
		ArrayList<OrderVO> orderList=new ArrayList<OrderVO>();
		List<Integer> oseqList=odao.getFinalListAll(id);
