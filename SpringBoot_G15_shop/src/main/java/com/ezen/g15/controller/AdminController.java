...
public class AdminController {
	...
	@ResponseBody //호출했던 위치로 되돌아가는 어노테이션
	public HashMap<String, Object> fileup(Model model, HttpServletRequest request){
	      ...
	      // ajax 에의해서 호출된 지점으로 다시 되돌아가서 화면 이동없이 운영이 계속되어야 하기때문에 이동할때
	      // 가져갈 데이터가 리턴됩니다. 따라서 리턴될 데이터들은 여러개가 있으므로 해시맵에 담겨서 리턴될 예정
	...
	public String productWrite(@ModelAttribute("dto") @Valid ProductVO productvo, BindingResult result,
	...
	public String productUpdate(@ModelAttribute("dto") @Valid ProductVO productvo, BindingResult result,
								Model model, HttpServletRequest request) {
	...
	public String orderUpdateResult(@RequestParam("result") int [] results){
		...
		//전달된 results 속에 담긴 odseq들을 하나씩 검색해서 result 값을 다음값으로 변경
